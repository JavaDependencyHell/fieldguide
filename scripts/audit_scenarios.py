...
# ---- Node part (same shape as Python: tool subdirs per scenario) ----
NODE_DIR = os.path.join(ROOT, "demos", "node-demo")
N_NODE = 12
if not os.path.isdir(NODE_DIR):
    note("FAIL", "node-demo directory missing")
else:
    node_verify = ""
    nvpath = os.path.join(NODE_DIR, "verify.sh")
    if os.path.exists(nvpath):
        node_verify = open(nvpath).read()
    else:
        note("FAIL", "node: no verify.sh")
    ndirs = {}
    for d in os.listdir(NODE_DIR):
        m = re.match(r"scenario-(\d+)-", d)
        if m:
            ndirs[int(m.group(1))] = os.path.join(NODE_DIR, d)
    for n in range(1, N_NODE + 1):
        ...
        if not os.path.exists(gpath):
            ...
            for inc, snip in re.findall(r'include="([^"]+)"\s+snippet="([^"]+)"', guide):
                ...
                if not os.path.exists(target):
                    ...
                    note("FAIL", f"{tag}: snippet marker '{snip}' not in {inc}")
            # whole-file includes (package.json etc. — JSON has no comments)
            for inc in re.findall(r'include="([^"]+)"(?!\s+snippet)', guide):
                if not os.path.exists(os.path.join(sdir, inc)):
                    note("FAIL", f"{tag}: guide includes missing file {inc}")
            for href in re.findall(r"\]\((\.\./[^)]+\.qmd)\)", guide):
                if not os.path.exists(os.path.normpath(os.path.join(sdir, href))):
                    note("FAIL", f"{tag}: broken Related link {href}")
        ...
        if f"{rel}/guide.qmd" not in quarto:
            note("FAIL", f"{tag}: guide not listed in _quarto.yml")
        if node_verify and not re.search(rf"Scenario {n}[ (]", node_verify) \
                and not re.search(rf'run_scenario {n} ', node_verify):
            note("WARN", f"{tag}: no check in node verify.sh")
...
# Node fixture registries (generated by install_node_deps.sh; under target/)
NODE_FIXTURES = [
    ("node-repo", "demo-lib-a/demo-lib-a-1.0.0.tgz"),
    ("node-repo", "demo-lib-c/demo-lib-c-1.1.0.tgz"),
    ("node-repo", "demo-plugin/demo-plugin-1.0.0.tgz"),
    ("node-repo", "acme-internal-tool/acme-internal-tool-9.9.9.tgz"),
    ("node-private-repo", "acme-internal-tool/acme-internal-tool-1.0.0.tgz"),
    ("node-private-repo", "@acme/internal-tool/internal-tool-1.0.0.tgz"),
]
for repo, f in NODE_FIXTURES:
    if not os.path.exists(os.path.join(ROOT, "target", repo, f)):
        note("FAIL", f"fixtures: target/{repo}/{f} missing — Node demos cannot resolve. Run install_node_deps.sh (expected after `make clean`).")
if not os.path.exists(os.path.join(ROOT, "target", "node-harness", "node_modules", ".bin", "verdaccio")):
    note("FAIL", "fixtures: target/node-harness verdaccio missing — run install_node_deps.sh (expected after `make clean`).")

print(f"Scenario matrix audit — {N_SCENARIOS} JVM scenarios x {len(TOOLS)} tools + {N_PY} Python scenarios + {N_NODE} Node scenarios")
print("-" * 60)
for p in problems:
    print(f"[FAIL] {p}")
for w in warnings:
